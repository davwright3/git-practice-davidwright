...
#Ensure valid population density calculate (handle missing data)
...
fig.add_trace(go.Choropleth(
    ...
    z=world["pop_density_log"].fillna(0),  #Fill missing data with 0
    colorscale="plasma",
    zmin=world["pop_density_log"].min(),
    zmax=world["pop_density_log"].max(),
    marker_line_color="black",
    marker_line_width=0.3,
    colorbar_title="Log (Population ensity)",
))

fig.update_geos(
    projection_type="orthographic", #3D Spehre projection
    showcoastlines=True,
    showland=True,
    showcountries=True,
)
